...
public class AppFileUtils {
	 ...
	 * @param houseimg
	 */
	public static String updateFileName(String houseimg,String suffix) {
		//找到文件
		try {
			File file=new File(PATH,houseimg);
			if(file.exists()) {
				file.renameTo(new File(PATH,houseimg.replace(suffix, "")));
				return houseimg.replace(suffix, "");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @param houseimg
	 */
	public static void removeFileByPath(String houseimg) {
		try {
			File file=new File(PATH,houseimg);
			if(file.exists()) {
				file.delete();
